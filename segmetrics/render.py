...
def plot_metrics(seg_metrics):
    ...
    for i, (sy, sx) in enumerate(tgt.bboxes):
    ...
    for fp in seg_metrics.false_positives:
      ...
      r = patches.Rectangle((sx.start,sy.start), w, h, edgecolor='r', facecolor=(1.,0.,0.,0.5), linewidth=2)
      ax.add_patch(r)
    ...
    for fn in seg_metrics.false_negatives:
      sy, sx = bboxes[fn-1]
      w, h = sx.stop-sx.start,sy.stop-sy.start
      r = patches.Rectangle((sx.start,sy.start), w, h, edgecolor='c', facecolor=(0.,1.,1.,0.5), linewidth=2)
      ax.add_patch(r)
